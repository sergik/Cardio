    ...
    public class GenerateEnemiesScript : GameEntityScript
    {
        ...
        public void GenerateEnemies()
        {
            ...
            for (int i = 0; i < enemiesToGenerateCount; i++)
            {
                ...
                var effectBox = cache.GetWithRemoveBlowByType(Particles.BlowType.EnemyGenerationBlow);

                if (effectBox != null)
                {
                    blow = effectBox.BlowEffect;
                }
                else
                {
                    blow = BlowEffect.FromMetadata(Game.Content.Load<BlowEffectMetadata>(@"Effects\Blows\EnemyGenerationBlow"), Game.Content);
                }
                ...
                var positionY = WorldPosition.Y + r.Next(200);
